/// Copied from `SegmentedList` source in Zig std library
/// renamed to `Xar` for shorter name, referencing [BSC talk by Andrew Reese](https://youtu.be/i-h95QIGchY?si=n_w0qkEt3ODLvNgb)
/// where an identicall data structure is described and is called `Xar`
...
pub fn Xar(comptime T: type, comptime prealloc_item_count: usize) type {
    return struct {
        const Self = @This();
        ...
        pub fn append(xar: *Self, arena: *Arena, item: T) Arena.AllocError!void {
            const new_item_ptr = try xar.add_one(arena);
            new_item_ptr.* = item;
        }

        pub fn append_slice(xar: *Self, arena: *Arena, items: []const T) Arena.AllocError!void {
            for (items) |item| {
                try xar.append(arena, item);
            }
        }

        pub fn pop(xar: *Self) ?T {
            if (xar.len == 0) return null;

            const index = xar.len - 1;
            const result = unchecked_at(xar, index).*;
            xar.len = index;
            return result;
        }

        pub fn add_one(xar: *Self, arena: *Arena) Arena.AllocError!*T {
            const new_length = xar.len + 1;
            try xar.grow_capacity(arena, new_length);
            const result = unchecked_at(xar, xar.len);
            xar.len = new_length;
            return result;
        }
        ...
        pub fn shrink_retaining_capacity(xar: *Self, new_len: usize) void {
            assert(new_len <= xar.len);
            xar.len = new_len;
        }

        /// Invalidates all element pointers.
        pub fn clear_retaining_capacity(xar: *Self) void {
            xar.len = 0;
        }

        /// Invalidates all element pointers.
        pub fn clear_and_free(xar: *Self, arena: *Arena) void {
            xar.set_capacity(arena, 0) catch unreachable;
            xar.len = 0;
        }

        /// Grows or shrinks capacity to match usage.
        /// TODO update this and related methods to match the conventions set by ArrayList
        pub fn set_capacity(xar: *Self, arena: *Arena, new_capacity: usize) Arena.AllocError!void {
            if (prealloc_item_count != 0) {
                if (new_capacity <= @as(usize, 1) << (prealloc_exp + @as(ShelfIndex, @intCast(xar.dynamic_segments.len)))) {
                    xar.shrink_capacity(arena, new_capacity);
                    return;
                }
            }
            return xar.grow_capacity(arena, new_capacity);
        }
        ...
        pub fn write_to_slice(xar: *Self, dest: []T, start: usize) void {
        ...
        pub fn unchecked_at(xar: anytype, index: usize) at_type(@TypeOf(xar)) {
            if (index < prealloc_item_count) {
                return &xar.prealloc_segment[index];
            }
            const shelf_index = get_shelf_index(index);
            const box_index = get_box_index(index, shelf_index);
            return &xar.dynamic_segments[shelf_index][box_index];
        }
        ...
        fn get_shelf_index(list_index: usize) ShelfIndex {
            if (prealloc_item_count == 0) {
        ...
        fn get_box_index(list_index: usize, shelf_index: ShelfIndex) usize {
            if (prealloc_item_count == 0) {
        ...
        fn BaseIterator(comptime SelfType: type, comptime ElementPtr: type) type {
            return struct {
                ...
                pub fn set(it: *@This(), index: usize) void {
                    ...
                    it.shelf_index = get_shelf_index(index);
                    it.box_index = get_box_index(index, it.shelf_index);
                    it.shelf_size = shelf_size(it.shelf_index);
                }
            };
        }

        pub fn iterator(xar: *Self, start_index: usize) Iterator {
            var it = Iterator{
                .list = xar,
                .index = undefined,
        ...
        pub fn const_iterator(xar: *const Self, start_index: usize) ConstIterator {
...
fn test_xar(comptime prealloc: usize) !void {
    var arena = try Arena.init(.{});
    defer arena.deinit();

    var list = Xar(i32, prealloc){};
    defer list.deinit(&arena);

    {
        var i: usize = 0;
        while (i < 100) : (i += 1) {
            try list.append(&arena, @as(i32, @intCast(i + 1)));
    ...
    try list.append_slice(&arena, &[_]i32{});
    try testing.expect(list.len == 99);

    {
        ...
        while (list.pop()) |item| : (i -= 1) {
            ...
            list.shrink_capacity(&arena, list.len);
        }
    }
...
test "clear_retaining_capacity" {
    ...
    try list.append_slice(&arena, &[_]i32{ 4, 5 });
    list.clear_retaining_capacity();
    try list.append(&arena, 6);
