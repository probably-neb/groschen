...
// Keep plaid adjacent but unused for now
...
const bg_color: Color = .{ .rgb = .{ 10, 10, 10 } };
...
// ---------------------------------------------------------------------------
// Side rails — drawn directly on grid after render
// ---------------------------------------------------------------------------

fn draw_side_rails(grid: *Grid) void {
    var row: u16 = 0;
    while (row < grid.rows) : (row += 1) {
        draw.write_cell(grid, 0, row, .{ .codepoint = unicode.box_light_vertical, .fg = rail_color, .bg = bg_color });
        draw.write_cell(grid, grid.cols -| 1, row, .{ .codepoint = unicode.box_light_vertical, .fg = rail_color, .bg = bg_color });
    }
}
...
fn build_ui() !void {
    ...
    // Spacer row 1 (top margin)
    ui.spacer(.y, 1);

    // Header row: GROSCHEN ... synced dot last update 2m ago
    try build_header();
    ...
    // Two-column layout for NET WORTH + ACCOUNTS, then CASH FLOW + EXPENSES
...
fn build_top_sections() !void {
    ...
    // Left column (37 cells, matching design cols 4-40)
    {
        ui.next_axis(.y);
    ...
    // Right column (36 cells, matching design cols 42-77)
    {
        ui.next_axis(.y);
...
// Persistent storage for expense bar custom draw data (up to 8 expenses)
